...
export default function QueryManagement() {
  ...
  const load = useCallback(async (overridePage?: number) => {
    setLoading(true);
    try {
      // request 拦截器在运行时把 res.data 解出来，但 axios 类型仍声明为 AxiosResponse<T>，此处直接 cast
  ...
  const onReset = () => {
    ...
    setDateRange(null);
    setPage(1);
    // 重置后立即刷新（清空再用旧值会有 race，改用 setTimeout 0 兜底）
    setTimeout(() => load(1), 0);
  };
  ...
  return (
    <Card title="AI 查询记录" extra={<Button icon={<ReloadOutlined />} onClick={() => load()} loading={loading}>刷新</Button>}>
